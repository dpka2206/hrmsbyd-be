import json

from rest_framework.response import Response

from workforce.exceptions.exceptions import (
    AlreadyClockedInException,
    EmployeeNotActiveException,
    NotClockedInException,
)
from workforce.presenters.attendance_presenter import AttendancePresenter
from workforce.views.clock_out.clock_out import clock_out
from workforce.views.clock_out.validator_class import ClockOutValidator


def execute(request):
    if hasattr(request, "data"):
        data = request.data
    else:
        data = json.loads(request.body.decode("utf-8")) if request.body else {}

    presenter = AttendancePresenter()
    validator = ClockOutValidator(data)
    try:
        input_dto = validator.validate()
    except ValueError as e:
        return Response(presenter.present_error(str(e)), status=400)

    try:
        output_dto = clock_out(input_dto)
        return Response(presenter.present_clock_out_success(output_dto), status=200)
    except EmployeeNotActiveException:
        return Response(
            presenter.present_error("Employee not found or not active."),
            status=404,
        )
    except NotClockedInException:
        return Response(
            presenter.present_error("Not clocked in. Cannot clock out."),
            status=404,
        )
    except AlreadyClockedInException:
        return Response(
            presenter.present_error("Already clocked out for today."),
            status=409,
        )
